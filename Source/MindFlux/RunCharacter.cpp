...
void ARunCharacter::Tick(float DeltaTime)
{
	...
	TimeRemainingFly -= DeltaTime;
	if(TimeRemainingFly < 0) TimeRemainingFly = 0;

	//UE_LOG(LogTemp, Log, TEXT("TimeRemainingFly: %f"), TimeRemainingFly);

	TimeRemainingHandwrist -= DeltaTime;
	if(TimeRemainingHandwrist < 0) TimeRemainingHandwrist = 0;

	//UE_LOG(LogTemp, Log, TEXT("TimeRemainingHandwrist: %f"), TimeRemainingHandwrist);

	if (TimeRemainingFly == 0 && isFlying) {
		EndFly();
	}

	if (TimeRemainingHandwrist == 0 && isHandwrist) {
		EndHandwrist();
	}
...
void ARunCharacter::ServerFly_OnTrigger_Implementation(bool isEnd) {
	...
	if (!isEnd && !isFlying) {
		...
		// Define how high the character should flyisHandwrist = true; and for how long.
		float FlyHeight = 300.0f; // Height above the ground
		float FlyDuration = 4.0f; // Duration of the flight in seconds
		TimeRemainingFly = 4.0f;
...
void ARunCharacter::SetHandwrist() {
	if (!isHandwrist) {
		...
		float Duration = 5.0f;
		TimeRemainingHandwrist = 5.0f;

		GetWorldTimerManager().SetTimer(HandwristTimerHandle, this, &ARunCharacter::EndHandwrist, Duration, false);
	}
}

void ARunCharacter::EndHandwrist() {
	isHandwrist = true;
	GetWorldTimerManager().ClearTimer(HandwristTimerHandle);
}
